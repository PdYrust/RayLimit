...
These overrides are set on the root command, before the subcommand, and apply to `limit` (a flag wins over its environment variable):

| Flag | Environment variable | Purpose |
| ---- | -------------------- | ------- |
| `--xray-binary <path>` | `RAYLIMIT_XRAY_BINARY` | Override the Xray binary path/name (see [Runtime Targeting](../core-concepts/runtime-targeting)) |
| `--container-cli <name>` | `RAYLIMIT_CONTAINER_CLI` | Container CLI: `docker` (default), `podman`, `nerdctl` |
...
The `limit`-specific flags are `--execute`, `--allow-missing-tc-state`, `--format`, and `--skip-privilege-check` (env `RAYLIMIT_SKIP_PRIVILEGE_CHECK`). `--skip-privilege-check` skips the up-front privilege check and trusts `CAP_NET_ADMIN`; real `tc` commands still enforce `EPERM`.
...
3. derives any runtime evidence the selected limiter family needs and observes current `tc` state, plus `nftables` state when needed
...
## Re-Applying Without Teardown

Applying one direction and then the other on the same device is now safe. For example:

```bash
sudo raylimit limit --pid 1234 --inbound api-in --device eth0 --direction upload --rate 2048 --execute
sudo raylimit limit --pid 1234 --inbound api-in --device eth0 --direction download --rate 2048 --execute
```

The second apply no longer fails with `RTNETLINK answers: File exists` and no longer tears down the sibling (upload) class and filter created by the first. Two mechanisms guarantee this:

- the mark-attachment apply path reconciles against the observed `tc`/`nftables` snapshot, so it only adds what is missing rather than blindly recreating the managed root; and
- an execution-time backstop treats an "already exists" / "busy" result on the managed root qdisc as an idempotent success (and takes over a foreign root with an explicit replace) instead of surfacing the error.

If you still see `File exists` on re-apply, you are on an older build — upgrade.
...
- `no_sessions` means RayLimit found no current client-IP evidence for that runtime, so there is no concrete per-IP work to plan or execute
