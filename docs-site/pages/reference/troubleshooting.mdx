...
## IP Baseline Does Not Behave Like Host-Global State

That is expected. `--ip all` is runtime-local. If another runtime still has unshaped traffic, that does not mean the baseline failed. It means the other runtime is outside the selected subject.
...
- removing a specific IP does not remove the baseline
- removing the baseline does not imply removal of every specific IP rule on other subjects
